...
- create/update the file versions1/latest-pre-release without new-line and setup 0.1.1 version folder
```
echo 0.1.1 > version.txt
mkdir versions1/0.1.1v
echo -n 0.1.1 > versions1/0.1.1v/latest-pre-release
cp versions1/certs.sector versions1/certs.sector.sig versions1/0.1.1v
cp versions1/public*key*   versions1/0.1.1v
```
...
idf.py app
mv build/LCM4ESP32.bin versions1/0.1.1v/otamain.bin
```
...
idf.py fullclean >/dev/null 2>&1; rm -rf /mnt/main
idf.py app
mv build/LCM4ESP32.bin versions1/0.1.1v/otaboot.bin

EXTRA_CFLAGS="-DOTABOOT -DOTABETA"
export EXTRA_CFLAGS
idf.py fullclean >/dev/null 2>&1; rm -rf /mnt/main
idf.py all
cp build/LCM4ESP32.bin versions1/0.1.1v/otabootbeta.bin
cp build/partition_table/partition-table.bin versions1/0.1.1v
cp build/bootloader/bootloader.bin versions1/0.1.1v
```
...
- commit and sync this as version 0.1.1  
- set up a new github release 0.1.1 as a pre-release using the just commited master...  
...
```
make flash OTAVERSION=0.1.1 OTABETA=1
```
...
```
openssl sha384 versions1/0.1.1v/otamain.bin
xxd versions1/0.1.1v/otamain.bin.sig
```

- upload the file versions1/0.1.1v/latest-pre-release to the 'latest release' assets on github
